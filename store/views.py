...
from django.db.models import Q, Sum, Avg, F, ExpressionWrapper, DecimalField, Count
...
from .models import Product, ProductView, Cart, CartItem, Order, OrderItem, Favorite, Category, Review, Notification, Address, ShippingOption, SellerProfile, Conversation, Message, SellerRating, UserProductView, Subscription, ProductRequest, Discount, GuineaRegion, GuineaPrefecture, GuineaQuartier, GuineaAddress, UserLocation, DeliveryLocation
import logging
from .forms import ProductForm, OrderStatusForm, ReviewForm, AddressForm, ApplyDiscountForm, SellerProfileForm, ProductRequestForm, ReportForm, ShippingMethodForm, GuineaAddressForm, UserLocationForm, DeliveryLocationForm, LocationSearchForm, CheckoutLocationForm, DeliveryAddressForm
...
def product_list(request, category_slug=None):
    ...
    # === SOLUTION CORRIGÉE ===
    if settings.DEBUG:  # Mode développement - affiche tous les produits
        print("[DEBUG] Mode développement - tous les produits visibles")
    else:
        if request.user.is_authenticated and hasattr(request.user, 'seller_profile'):
            # Vendeur connecté - voit tous SES produits + les produits approuvés
            products = products.filter(
                Q(seller=request.user) | Q(moderation__status='approved'))
        else:
            # Acheteur ou non connecté - seulement les produits approuvés
            products = products.filter(moderation__status='approved')
    ...
    # Debug
    print(f"[DEBUG] Utilisateur: {request.user.username if request.user.is_authenticated else 'Anonyme'}")
    print(f"[DEBUG] Produits visibles: {products.count()}/{Product.objects.count()}")
...
def add_address(request):
    ...
    next_url = request.GET.get('next', reverse('store:checkout'))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Si c'est la première adresse ou si l'utilisateur a coché "par défaut"
            if form.cleaned_data.get('is_default') or not Address.objects.filter(user=request.user).exists():
                address.is_default = True
            
            address.save()
            
            # Si cette adresse est marquée comme par défaut, on met à jour les autres
...
def create_location(request):
    """Créer une nouvelle localisation utilisateur"""
    if request.method == 'POST':
        form = UserLocationForm(request.POST, request.FILES)
        delivery_form = DeliveryLocationForm(request.POST)
        ...
        if form.is_valid():
            ...
            # Créer automatiquement une adresse de livraison si demandé
            if request.POST.get('create_delivery') and delivery_form.is_valid():
                delivery = delivery_form.save(commit=False)
                delivery.user_location = location
                delivery.save()
            
            messages.success(request, 'Localisation créée avec succès!')
            return redirect('store:my_locations')
    else:
        form = UserLocationForm()
        delivery_form = DeliveryLocationForm()
    
    return render(request, 'store/create_location.html', {
        'form': form,
...
def my_locations(request):
    """Liste des localisations de l'utilisateur"""
    locations = UserLocation.objects.filter(user=request.user, is_active=True)
    return render(request, 'store/geolocation/my_locations.html', {'locations': locations})
...
def search_addresses(request):
    ...
    addresses = GuineaAddress.objects.filter(status='validated')
    
    if query:
        addresses = addresses.filter(
            Q(description__icontains=query) |
            Q(landmark__icontains=query) |
            Q(quartier__name__icontains=query) |
            Q(prefecture__name__icontains=query)
        )
    
    if region_id:
        addresses = addresses.filter(region_id=region_id)
    
    addresses = addresses.order_by('-usage_count')[:10]
...
def get_prefectures(request):
    ...
    prefectures = GuineaPrefecture.objects.filter(region_id=region_id).values('id', 'name')
    return JsonResponse({'prefectures': list(prefectures)})

@csrf_exempt
def get_quartiers(request):
    """API pour récupérer les quartiers d'une préfecture"""
    prefecture_id = request.GET.get('prefecture_id')
    quartiers = GuineaQuartier.objects.filter(prefecture_id=prefecture_id).values('id', 'name')
...
def reverse_geocode(request):
    ...
    if request.method == 'POST':
        ...
        # Rechercher l'adresse la plus proche dans notre base
        closest_address = GuineaAddress.objects.filter(
            status='validated',
        ...
        if closest_address:
            return JsonResponse({
                'success': True,
                'address': {
                    'id': closest_address.id,
                    'description': closest_address.description,
                    'full_address': closest_address.full_address,
                    'region': closest_address.region.name,
                    'prefecture': closest_address.prefecture.name,
                    'quartier': closest_address.quartier.name
                }
            })
        
        # Si aucune adresse proche, utiliser Nominatim
        try:
...
@login_required
def contribute_address(request):
    """Contribuer une nouvelle adresse"""
    if request.method == 'POST':
        form = GuineaAddressForm(request.POST, request.FILES)
        if form.is_valid():
            address = form.save(commit=False)
            address.created_by = request.user
            address.save()
            messages.success(request, 'Adresse soumise pour validation!')
            return redirect('store:address_list')
    else:
        form = GuineaAddressForm()
    
    return render(request, 'store/geolocation/contribute_address.html', {'form': form})

def address_list(request):
    """Liste publique des adresses validées"""
    ...
    addresses = addresses.order_by('-usage_count', '-created_at')
    paginator = Paginator(addresses, 20)
    page = request.GET.get('page')
    addresses = paginator.get_page(page)
    
    return render(request, 'store/geolocation/address_list.html', {
        'addresses': addresses,
        'form': form
    })
...
def address_detail(request, address_id):
    ...
    address = get_object_or_404(GuineaAddress, id=address_id, status='validated')
    address.increment_usage()
    
    return render(request, 'store/geolocation/address_detail.html', {'address': address})

@login_required
def checkout(request):
    """Page de checkout avec géolocalisation intégrée"""
    ...
    if request.method == 'POST':
        location_form = CheckoutLocationForm(request.POST, request.FILES)
        delivery_form = DeliveryAddressForm(request.POST)
        shipping_option_form = ShippingMethodForm(request.POST)
        
        if location_form.is_valid() and delivery_form.is_valid() and shipping_option_form.is_valid():
            # Créer la localisation utilisateur
            ...
            # Stocker l'ID de la localisation en session pour le paiement
            request.session['delivery_location_id'] = delivery.id
            
            messages.success(request, 'Informations de livraison enregistrées!')
            return redirect('store:process_payment')
    else:
        location_form = CheckoutLocationForm()
        delivery_form = DeliveryAddressForm()
        shipping_option_form = ShippingMethodForm()
    ...
    if not addresses.exists():
        ...
        add_address_url = reverse('store:add_address')
        return redirect(f"{add_address_url}?next={reverse('store:checkout')}")
...
def process_payment(request):
    ...
    if not location_form.is_valid() or not request.POST.get('latitude') or not request.POST.get('longitude'):
        messages.error(request, "Veuillez définir une localisation valide avec latitude et longitude.")
        return redirect('store:checkout')
    ...
    if location.photo:
        ...
        elif not (request.POST.get('latitude') and request.POST.get('longitude')):
            messages.error(request, "Aucune coordonnée GPS trouvée dans la photo. Veuillez définir une localisation manuellement.")
            return redirect('store:checkout')
    location.user = request.user
    ...
    if payment_method == 'card' and payment_method_id:
        ...
        logger.info(f"Paiement à la livraison (COD) pour la commande #{order.id}")
        order.charge_id = None
        order.status = 'pending'
        order.save()
...
def apply_promo_code(request):
    try:
        ...
        try:
            code = PromoCode.objects.get(code=promo_code)
            logger.debug(f"Promo code: {code}, valid_from={code.valid_from}, valid_to={code.valid_to}, uses={code.uses}, max_uses={code.max_uses}, user_in_users={request.user in code.users.all()}")
            if not code.is_valid(user=request.user):
                logger.debug("Promo code invalid")
