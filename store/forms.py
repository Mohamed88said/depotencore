...
# === Formulaires de géolocalisation ===
class GuineaAddressForm(forms.ModelForm):
    class Meta:
        model = GuineaAddress
        fields = ['region', 'prefecture', 'quartier', 'description', 'landmark', 'latitude', 'longitude', 'photo']
        widgets = {
            'region': forms.Select(attrs={'class': 'form-control', 'id': 'id_region'}),
            'prefecture': forms.Select(attrs={'class': 'form-control', 'id': 'id_prefecture'}),
            'quartier': forms.Select(attrs={'class': 'form-control', 'id': 'id_quartier'}),
            'description': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 3,
                'placeholder': 'Ex: 123 près de la mosquée Diaouné, en face du marché'
            }),
            'landmark': forms.TextInput(attrs={
                'class': 'form-control',
                'placeholder': 'Ex: Mosquée, École, Marché'
            }),
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['prefecture'].queryset = GuineaPrefecture.objects.none()
        self.fields['quartier'].queryset = GuineaQuartier.objects.none()
        
        if 'region' in self.data:
            try:
                region_id = int(self.data.get('region'))
                self.fields['prefecture'].queryset = GuineaPrefecture.objects.filter(region_id=region_id)
            except (ValueError, TypeError):
                pass
        elif self.instance.pk:
            self.fields['prefecture'].queryset = self.instance.region.prefectures.all()
            self.fields['quartier'].queryset = self.instance.prefecture.quartiers.all()
        
        if 'prefecture' in self.data:
            try:
                prefecture_id = int(self.data.get('prefecture'))
                self.fields['quartier'].queryset = GuineaQuartier.objects.filter(prefecture_id=prefecture_id)
            except (ValueError, TypeError):
                pass
    ...
    class Meta:
        ...
        widgets = {
            'description': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 3,
                'placeholder': 'Décrivez précisément votre localisation'
            }),
            'photo': forms.ClearableFileInput(attrs={'class': 'form-control'}),
            'latitude': forms.HiddenInput(),
            'longitude': forms.HiddenInput(),
...
class DeliveryLocationForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['access_instructions', 'contact_phone', 'is_default']
        widgets = {
            'access_instructions': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 3,
                'placeholder': 'Comment accéder à cette adresse (étage, porte, etc.)'
            }),
...
class LocationSearchForm(forms.Form):
    ...
    region = forms.ModelChoiceField(
        queryset=GuineaRegion.objects.all(),
        required=False,
...
class CheckoutLocationForm(forms.ModelForm):
    """Formulaire de localisation pour le checkout"""
    class Meta:
        model = UserLocation
        fields = ['description', 'photo', 'latitude', 'longitude']
        widgets = {
            'description': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 3,
                'placeholder': 'Décrivez précisément votre localisation pour la livraison'
            }),
            'photo': forms.ClearableFileInput(attrs={'class': 'form-control'}),
            'latitude': forms.HiddenInput(),
            'longitude': forms.HiddenInput(),
...
class DeliveryAddressForm(forms.ModelForm):
    ...
    class Meta:
        model = DeliveryLocation
        fields = ['access_instructions', 'contact_phone']
        widgets = {
            'access_instructions': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 2,
                'placeholder': 'Instructions pour accéder à votre adresse'
            }),
            'contact_phone': forms.TextInput(attrs={
                'class': 'form-control',
                'placeholder': '+224 XX XX XX XX'
